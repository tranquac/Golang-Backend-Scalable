...
func NewCustomError(root error, msg, key string) *AppError {
	if root != nil {
		NewErrorResponse(root, msg, root.Error(), key)
	}
...
func ErrInternal(err error) *AppError {
	return NewFullErrorResponse(http.StatusInternalServerError, err, "something went wrong in server", err.Error(), "ErrInternal")
}
...
func ErrCannotGetEntity(entity string, err error) *AppError {
	return NewCustomError(err, fmt.Sprintf("Cannot get %s", strings.ToLower(entity)), fmt.Sprintf("ErrGetUpdate%s", entity))
}
